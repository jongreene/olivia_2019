...
def clock():
	...
	draw.text((0, 10), 'You are', fill = "WHITE", font = font)
	draw.text((0, 34), 'Beautiful', fill = "WHITE",font = font)
	draw.text((0, 70), currentDT.strftime("%H:%M"), fill = "WHITE",font = font)
	draw.text((40, 90), currentDT.strftime("%S"), fill = "WHITE",font = font)
	OLED.Display_Image(image)
	OLED.Delay(1)
